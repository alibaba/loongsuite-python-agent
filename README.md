...
In **Release**, published packages link to **PyPI** (URL pattern `https://pypi.org/project/{distribution-name}/`); packages not yet on PyPI show **in dev**.
...
- **loongsuite-site-bootstrap** (optional) — [https://pypi.org/project/loongsuite-site-bootstrap/](https://pypi.org/project/loongsuite-site-bootstrap/).
...
**Step 3 — Call the framework instrumentor** after Step 2, then start your app—still without `loongsuite-instrument`.
...
**Step 3 — Install LoongSuite components in this order** (so `loongsuite-util-genai` resolves to the local build, not the upstream util):
...
**Step 4 — Install the instrumentations you need**, for example:

  ```bash
  pip install -e ./instrumentation-loongsuite/loongsuite-instrumentation-agentscope
  ```

**Step 5 — Run under `loongsuite-instrument`**
