...
def load_skip_config(config_path: Path) -> Set[str]:
    ...
    logger.info(
        f"Loaded {len(skip_packages)} packages to skip from config file: {skip_packages}"
    )
...
def collect_packages(
    ...
    for whl_file in all_whl_files:
        ...
        if package_name in skip_packages:
            logger.info(
                f"Skipping package: {package_name} (according to config file)"
            )
            skipped_count += 1
        ...
        if package_name in seen_packages:
            ...
            if whl_file.stat().st_mtime > existing_file.stat().st_mtime:
                logger.debug(
                    f"Replacing duplicate package {package_name}: {existing_file.name} -> {whl_file.name}"
                )
                ...
                logger.debug(
                    f"Skipping older version {package_name}: {whl_file.name}"
                )
                whl_file.unlink()
        else:
            seen_packages[package_name] = whl_file
...
def create_tar_archive(whl_files: List[Path], output_path: Path):
    ...
    logger.info(
        f"Successfully created archive: {output_path} ({output_path.stat().st_size / 1024 / 1024:.2f} MB)"
    )
