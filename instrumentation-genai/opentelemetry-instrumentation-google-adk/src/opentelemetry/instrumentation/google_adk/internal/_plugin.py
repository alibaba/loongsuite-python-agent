...
from ._utils import (
    safe_json_dumps, safe_json_dumps_for_input_output, extract_content_safely_for_input_output,
...
class GoogleAdkObservabilityPlugin(BasePlugin):
    ...
    async def before_run_callback(
        ...
        try:
            ...
            attributes = self._extractors.extract_runner_attributes(invocation_context)
            ...
            self._active_spans[f"runner_{invocation_context.invocation_id}"] = span
            ...
            if runner_key in self._runner_inputs and should_capture_content():
                ...
                if input_messages:
                    ...
                    span.set_attribute("input.value", safe_json_dumps_for_input_output(input_messages))
                    _logger.debug(f"Set input.value on Agent span: {invocation_context.invocation_id}")
    ...
    async def on_user_message_callback(
        ...
        try:
            ...
            _logger.debug(f"Captured user message for Runner: {invocation_context.invocation_id}")
    ...
    async def on_event_callback(
        ...
        try:
            ...
            if hasattr(event, 'content') and event.content:
                event_content = extract_content_safely_for_input_output(event.content)
            elif hasattr(event, 'data') and event.data:
                event_content = extract_content_safely_for_input_output(event.data)
            ...
            if event_content:
                ...
                if span:
                    output_messages = [{
                        ...
                        "parts": [{
                            "type": "text", 
                            "content": process_content(self._runner_outputs[runner_key])
                        }],
                        "finish_reason": "stop"
                    }]
                    
                    # For Agent spans, use output.value
                    span.set_attribute("output.value", safe_json_dumps_for_input_output(output_messages))
    ...
    async def after_run_callback(
        ...
        try:
            ...
            if span:
                ...
                conversation_id = invocation_context.session.id if invocation_context.session else None
                ...
                span.end()
                _logger.debug(f"Finished Runner span for {invocation_context.app_name}")
    ...
    async def before_agent_callback(
        ...
        try:
            ...
            attributes = self._extractors.extract_agent_attributes(agent, callback_context)
    ...
    async def after_model_callback(
        ...
        try:
            ...
            if llm_span:
                ...
                response_attrs = self._extractors.extract_llm_response_attributes(llm_response)
                ...
                model_name = self._resolve_model_name(llm_response, request_key, llm_span)
    ...
    async def after_tool_callback(
        ...
        try:
            ...
            if span:
                ...
                conversation_id = getattr(tool_context, 'session_id', None) if tool_context else None
    ...
    def _calculate_span_duration(self, span: trace_api.Span) -> float:
        ...
        if hasattr(span, 'start_time') and span.start_time:
            ...
            return (current_time_ns - span.start_time) / 1_000_000_000  # ns to s
        return 0.0
    ...
    def _resolve_model_name(
        ...
        if llm_response and hasattr(llm_response, 'model') and getattr(llm_response, 'model'):
            model_name = getattr(llm_response, 'model')
            
        # 2) Use stored request model by request_key
        if not model_name and request_key and request_key in self._llm_req_models:
            model_name = self._llm_req_models.pop(request_key, None)
            
        # 3) Try span attributes if accessible
        if not model_name and hasattr(span, 'attributes') and getattr(span, 'attributes'):
            model_name = span.attributes.get("gen_ai.request.model")
            
        # 4) Parse from span name like "chat <model>"
        if not model_name and hasattr(span, 'name') and isinstance(span.name, str):
            try:
                name = span.name
    ...
    def _convert_user_message_to_genai_format(self, user_message: types.Content) -> list:
        """
        ...
        """
        input_messages = []
        if user_message and hasattr(user_message, 'role') and hasattr(user_message, 'parts'):
            message = {
