...
class OTelGenAIMetricsValidator:
    ...
    def validate_metrics_data(self, metric_reader: InMemoryMetricReader) -> Dict[str, Any]:
            ...
            for scope_metrics in resource_metrics.scope_metrics:
                for metric in scope_metrics.metrics:
                    ...
                    if metric.name in self.NON_STANDARD_METRICS:
                        validation_result["non_standard_found"].add(metric.name)
    ...
    def _validate_single_metric(self, metric) -> Dict[str, Any]:
        ...
        for data_point in data_points:
            point_validation = self._validate_data_point(metric.name, data_point)
    ...
    def _validate_data_point(self, metric_name: str, data_point) -> Dict[str, Any]:
        ...
        if hasattr(data_point, 'attributes'):
            result["attributes"] = dict(data_point.attributes) if data_point.attributes else {}
        ...
        if "gen_ai.operation.name" not in attributes:
            result["errors"].append("Missing required attribute: gen_ai.operation.name")
        
        if "gen_ai.provider.name" not in attributes:
            result["errors"].append("Missing required attribute: gen_ai.provider.name")
        
        # Check for non-standard attributes
        non_standard_attrs = {
            "callType", "callKind", "rpcType", "spanKind",  # ARMS attributes
            "modelName", "usageType",  # Should be gen_ai.request.model, gen_ai.token.type
            "session_id", "user_id"  # High cardinality, should not be in metrics
        }
        
        for attr in non_standard_attrs:
            if attr in attributes:
                result["errors"].append(f"Found non-standard attribute: {attr}")
        ...
        if "gen_ai.token.type" in attributes:
            ...
            if token_type not in ["input", "output"]:
                result["errors"].append(f"Invalid gen_ai.token.type value: {token_type}")
...
class TestGoogleAdkMetricsIntegration:
    ...
    async def test_llm_metrics_with_standard_otel_attributes(self):
        ...
        validation_result = self.validator.validate_metrics_data(self.metric_reader)
        ...
        assert "gen_ai.client.operation.duration" in validation_result["metrics_found"], \
            "Should have gen_ai.client.operation.duration metric"
        assert "gen_ai.client.token.usage" in validation_result["metrics_found"], \
            "Should have gen_ai.client.token.usage metric"
        
        # Get actual data points
        duration_points = self.get_metric_data_points("gen_ai.client.operation.duration")
        assert len(duration_points) >= 1, "Should have at least 1 duration data point"
        ...
        assert "session_id" not in duration_attrs, "Should NOT have session_id (high cardinality)"
        assert "user_id" not in duration_attrs, "Should NOT have user_id (high cardinality)"
        
        # Get token usage data points
        token_points = self.get_metric_data_points("gen_ai.client.token.usage")
        assert len(token_points) == 2, "Should have 2 token usage data points (input + output)"
        
        # Validate token types
        token_types = {dict(dp.attributes).get("gen_ai.token.type") for dp in token_points}
        assert token_types == {"input", "output"}, \
            "Should have both input and output token types"
        
        # Validate token values
        input_point = [dp for dp in token_points 
                       if dict(dp.attributes).get("gen_ai.token.type") == "input"][0]
        output_point = [dp for dp in token_points 
                        if dict(dp.attributes).get("gen_ai.token.type") == "output"][0]
        ...
        assert "usageType" not in input_attrs, "Should NOT have usageType (use gen_ai.token.type)"
    ...
    async def test_agent_metrics_use_standard_attributes(self):
        ...
        assert "gen_ai.provider.name" in agent_attrs, "Should have provider name"
        
        # Agent name should be in gen_ai.request.model
        assert agent_attrs.get("gen_ai.request.model") == "math_tutor" or \
               "gen_ai.agent.name" in agent_attrs, \
            "Agent name should be in metrics"
        
        # Validate NO ARMS attributes
        assert "spanKind" not in agent_attrs, "Should NOT have spanKind"
        assert "session_id" not in agent_attrs, "Should NOT have high-cardinality session_id"
        assert "user_id" not in agent_attrs, "Should NOT have high-cardinality user_id"
    ...
    async def test_tool_metrics_use_standard_attributes(self):
        ...
        assert tool_attrs.get("gen_ai.request.model") == "calculator" or \
               "gen_ai.tool.name" in tool_attrs, \
            "Tool name should be in metrics"
    ...
    async def test_only_two_standard_metrics_recorded(self):
        ...
        standard_metrics = validation_result["metrics_found"] & self.validator.STANDARD_METRICS
        ...
        arms_metrics = {
            "calls_count", "calls_duration_seconds", "call_error_count",
            "llm_usage_tokens", "llm_first_token_seconds"
        }
        ...
        custom_genai_metrics = {
            "genai_calls_count", "genai_calls_duration_seconds",
            "genai_calls_error_count", "genai_calls_slow_count",
            "genai_llm_first_token_seconds", "genai_llm_usage_tokens"
        }
        found_custom_metrics = validation_result["metrics_found"] & custom_genai_metrics
