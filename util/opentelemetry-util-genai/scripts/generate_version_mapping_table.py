...
def _render_generated_block(data: dict[str, Any], generated_at: str) -> str:
    ...
    lines.append(".. This section is generated by scripts/generate_version_mapping_table.py")
    lines.append("")
    ...
    if not mappings:
        ...
        for item in mappings:
            ...
            lines.append(f"   * - ``{_format_cell(item.get('loongsuite_version'))}``")
            lines.append(f"     - ``{_format_cell(item.get('upstream_version'))}``")
            lines.append(f"     - ``{_format_cell(item.get('upstream_commit'))}``")
...
def main() -> None:
    ...
    parser.add_argument(
        "--mapping",
        type=Path,
        default=Path(__file__).resolve().parent.parent / "upstream_version_map.json",
        help="Path to upstream version mapping JSON file.",
    )
    parser.add_argument(
        "--readme",
        type=Path,
        default=Path(__file__).resolve().parent.parent / "README-loongsuite.rst",
