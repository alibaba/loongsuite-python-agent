...
class AdkAttributeExtractors:
    ...
    def extract_runner_attributes(
        ...
        try:
            ...
            try:
                ...
                _logger.debug(
                    "Failed to extract conversation_id from invocation_context"
                )
            ...
            try:
                ...
                if not user_id and hasattr(invocation_context, "session"):
                    user_id = getattr(
                        invocation_context.session, "user_id", None
                    )
            except AttributeError:
                _logger.debug(
                    "Failed to extract user_id from invocation_context"
                )

            if conversation_id is None:
                _logger.debug(
                    "conversation_id not found on invocation_context"
                )
            if user_id is None:
    ...
    def extract_agent_attributes(
        ...
        try:
            ...
            try:
                ...
                _logger.debug(
                    "Failed to extract conversation_id from callback_context"
                )
            ...
            try:
                ...
                _logger.debug(
                    "Failed to extract user_id from callback_context"
                )
    ...
    def extract_llm_request_attributes(
        ...
        try:
            ...
            if hasattr(llm_request, "model") and llm_request.model:
                ...
                attrs["gen_ai.provider.name"] = self._extract_provider_name(
                    llm_request.model
                )
            ...
            if hasattr(llm_request, "config") and llm_request.config:
                ...
                if (
                    ...
                    if isinstance(config.temperature, (int, float)):
                        attrs["gen_ai.request.temperature"] = (
                            config.temperature
                        )
            ...
            if (
                ...
                try:
                    ...
                    for content in llm_request.contents:
                        if hasattr(content, "role") and hasattr(
                            content, "parts"
                        ):
                    ...
                    if input_messages:
                        attrs["gen_ai.input.messages"] = safe_json_dumps_large(
                            input_messages
                        )
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    if (
                        hasattr(llm_response, "text")
                        and llm_response.text is not None
                    ):
                        extracted_text = (
                            extract_content_safely_for_input_output(
                                llm_response.text
                    ...
                    if output_messages:
                        attrs["gen_ai.output.messages"] = (
                            safe_json_dumps_large(output_messages)
                        )
    ...
    def extract_tool_attributes(
        ...
        try:
            ...
            elif hasattr(tool_context, "context") and hasattr(
                tool_context.context, "session_id"
            ):
            ...
            if should_capture_content() and tool_args:
                attrs["gen_ai.tool.call.arguments"] = safe_json_dumps(
                    tool_args
                )
                attrs["input.value"] = safe_json_dumps_for_input_output(
                    tool_args
                )
                attrs["input.mime_type"] = "application/json"
