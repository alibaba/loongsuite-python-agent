    ...
    @pytest.mark.parametrize(
        ...
        [
            # 测试dict类型的prompt - 包含role和text
            {
                ...
                "expected": [
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.0.{MessageAttributes.MESSAGE_ROLE}",
                        "user",
                    ),
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.0.{MessageAttributes.MESSAGE_CONTENT}",
                        "Hello, how are you?",
                    ),
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.1.{MessageAttributes.MESSAGE_ROLE}",
                        "assistant",
                    ),
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.1.{MessageAttributes.MESSAGE_CONTENT}",
            ...
            # 测试dict类型的prompt - 只包含text
            {
                ...
                "inputs": {"prompts": [{"text": "This is a system message"}]},
                "expected": [
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.0.{MessageAttributes.MESSAGE_CONTENT}",
            ...
            # 测试string类型的prompt
            {
                ...
                "inputs": {"prompts": ["Hello, this is a simple prompt"]},
                "expected": [
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.0.{SpanAttributes.CONTENT}",
            ...
            # 测试string类型的prompt - 多个
            {
                ...
                "expected": [
                    ...
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.1.{SpanAttributes.CONTENT}",
                        "Second prompt",
                    ),
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.2.{SpanAttributes.CONTENT}",
                        "Third prompt",
                    ),
                ],
            },
            # 测试混合类型 - dict和string
            {
                ...
                "expected": [
                    ...
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.2.{MessageAttributes.MESSAGE_ROLE}",
                        "assistant",
                    ),
                    (
                        f"{SpanAttributes.GEN_AI_PROMPT}.2.{MessageAttributes.MESSAGE_CONTENT}",
                        "Assistant response",
                    ),
                ],
            },
            ...
            {
                "name": "no_prompts_key",
                "inputs": {"messages": ["some message"]},
                "expected": [],
            },
            # 测试边界情况 - None输入
            {"name": "none_input", "inputs": None, "expected": []},
            # 测试边界情况 - 空dict输入
            {"name": "empty_dict_input", "inputs": {}, "expected": []},
            # 测试边界情况 - 非Mapping类型输入
            {
    ...
    def test_prompts_with_special_characters(self):
        ...
        inputs = {
            "prompts": [
                ...
                {
                    "role": "assistant",
                    "text": "Response with unicode: 你好世界",
                },
            ]
        }
