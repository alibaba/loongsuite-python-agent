...
class TestSyncLLMCalls(TestBase):
    ...
    def test_basic_sync_crew_execution(self):
        ...
        self.assertGreaterEqual(
            len(spans), 3, f"Expected at least 3 spans, got {len(spans)}"
        )
        ...
        self.assertEqual(
            agent_span.attributes.get("gen_ai.span.kind"), "AGENT"
        )
        ...
        if llm_spans:
            ...
            self.assertEqual(
                llm_span.attributes.get("gen_ai.system"), "dashscope"
            )
            self.assertIsNotNone(
                llm_span.attributes.get("gen_ai.request.model")
            )
            self.assertGreater(
                ...
                for metric in scope_metrics.metrics:
                    if metric.name == "genai_calls_count":
                        ...
                        for data_point in metric.data.data_points:
                            ...
                            self.assertIsNotNone(
                                data_point.attributes.get("modelName")
                            )
                            self.assertEqual(
                                data_point.attributes.get("spanKind"), "LLM"
                            )
        ...
        self.assertTrue(
            calls_count_found, "genai_calls_count metric not found"
        )
        self.assertTrue(
                    ...
                    if metric.name == "genai_calls_count":
                        for data_point in metric.data.data_points:
                            ...
                            self.assertGreaterEqual(
                                data_point.value,
                                2,
                                f"Expected at least 2 LLM calls, got {data_point.value}",
                            )
