...
class TestPromptAndMemory(TestBase):
    ...
    def test_prompt_template_with_variables(self):
        ...
        if task_spans:
            ...
            self.assertTrue(
                "Analyze" in input_value or "analyze" in input_value.lower()
            )
    ...
    def test_agent_with_memory(self):
        ...
        retriever_spans = [
            s
            for s in spans
            if s.attributes.get("gen_ai.span.kind") == "RETRIEVER"
        ]
        ...
        if retriever_spans:
            ...
            self.assertEqual(
                retriever_span.attributes.get("gen_ai.span.kind"), "RETRIEVER"
            )
