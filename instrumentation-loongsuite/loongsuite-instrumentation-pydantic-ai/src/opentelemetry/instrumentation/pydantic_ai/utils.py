...
"""Utility functions for Pydantic AI instrumentation."""
...
OP_NAME_AGENT_RUN = "invoke_agent"
OP_NAME_TOOL = "execute_tool"

GEN_AI_SYSTEM = "pydantic-ai"


class GenAIHookHelper:
    """Helper for recording GenAI content on spans following LoongSuite conventions."""
...
def to_input_message(role: str, content: Any) -> List[InputMessage]:
    """Convert content to a list of InputMessage objects."""
    if not content:
        return []
...
def to_output_message(
    ...
    """Convert content to a list of OutputMessage objects."""
    if not content:
        return []
...
def extract_agent_run_inputs(
    user_prompt: Any,
    instructions: Any = None,
) -> List[InputMessage]:
    """Extract input messages from agent run parameters."""
    parts = []

    if user_prompt:
        prompt_str = user_prompt if isinstance(user_prompt, str) else str(user_prompt)
        parts.append(Text(content=prompt_str))

    if not parts:
        return []

    messages = [InputMessage(role="user", parts=parts)]

    if instructions:
        instr_str = instructions if isinstance(instructions, str) else str(instructions)
        system_parts = [Text(content=instr_str)]
        messages.insert(0, InputMessage(role="system", parts=system_parts))

    return messages


def extract_tool_inputs(tool_name: str, arguments: Any) -> List[InputMessage]:
    """Extract input messages from tool call parameters."""
    args_str = (
