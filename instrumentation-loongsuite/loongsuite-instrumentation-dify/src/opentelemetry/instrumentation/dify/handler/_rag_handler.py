...
class ToolInvokeHandler(TOOLBaseWrapper):
    ...
    def _get_output_attributes(
        ...
        if error:
            ...
            if tool_config:
                ...
                if tool_parameters:
                    set_dict_value(
                        attributes, "tool.parameters", str(tool_parameters)
                    )
    ...
    def __call__(
        ...
        with self.tracer.start_as_current_span(
            ...
            try:
                tool_invoke_response, tool_invoke_meta = wrapped(
                    *args, **kwargs
                )
            except Exception as e:
                try:
                    output_attributes, _, _ = self._get_output_attributes(
                        None, e
                    )
            ...
            try:
                ...
                output_attributes, time_cost, span_kind = (
                    self._get_output_attributes(
                ...
                self.record_call_count(
                    tool_name=tool_name, span_kind=span_kind
                )
...
class RetrieveHandler(BaseWrapper):
    ...
    def _get_input_attributes(self, args, kwargs):
        ...
        if document_ids_filter:
            attributes["retrieval.document_ids_filter"] = str(
                document_ids_filter
            )
        return attributes
        ...
        if isinstance(result, list):
            for i, doc in enumerate(result):
                ...
                if (
                    ...
                    and "document_id" in doc.metadata
                ):
                    attributes[f"retrieval.documents.{i}.document.id"] = (
                ...
                if (
                    hasattr(doc, "metadata")
                    and isinstance(doc.metadata, dict)
                    and "score" in doc.metadata
                ):
                    attributes[f"retrieval.documents.{i}.document.score"] = (
                        doc.metadata["score"]
                    )
    ...
    def __call__(
        ...
        with self.tracer.start_as_current_span(
            ...
            try:
                ...
                logger.debug(
                    "Failed to set input attributes in RetrieveHandler",
                    exc_info=True,
                )
            ...
            try:
                ...
                self.record_duration(
                    duration=duration, span_kind=SpanKindValues.RETRIEVER.value
                )
                context_attributes = self.extract_attributes_from_context()
                span.set_attributes(context_attributes)
            ...
            try:
                ...
                logger.debug(
                    "Failed to set output attributes in RetrieveHandler",
                    exc_info=True,
                )
...
class VectorSearchHandler(BaseWrapper):
    ...
    def _get_input_attributes(self, args, kwargs):
        ...
        if query:
            set_dict_value(attributes, "vector_search.query", query)
        ...
        if isinstance(result, list):
            for i, doc in enumerate(result):
                if hasattr(doc, "page_content"):
                    set_dict_value(
                        attributes,
                        f"vector_search.document.{i}.page_content",
                        doc.page_content,
                    )
                if hasattr(doc, "vector") and doc.vector is not None:
                    attributes[f"vector_search.document.{i}.vector_size"] = (
                        len(doc.vector)
                    )
                if hasattr(doc, "provider"):
                    attributes[f"vector_search.document.{i}.provider"] = (
                        doc.provider
                    )
                if hasattr(doc, "metadata"):
                    attributes[f"vector_search.document.{i}.metadata"] = str(
                        doc.metadata
                    )
        return attributes
    ...
    def __call__(
        ...
        with self.tracer.start_as_current_span(
            ...
            try:
                ...
                logger.debug(
                    "Failed to set input attributes in VectorSearchHandler",
                    exc_info=True,
                )
...
class FullTextSearchHandler(BaseWrapper):
    ...
    def _get_input_attributes(self, args, kwargs):
        ...
        query = args[0] if args else kwargs.get("query")
        if query:
            set_dict_value(attributes, "full_text_search.query", query)
        for key, value in kwargs.items():
        ...
        if isinstance(result, list):
            for i, doc in enumerate(result):
                if hasattr(doc, "page_content"):
                    set_dict_value(
                        attributes,
                        f"full_text_search.document.{i}.page_content",
                        doc.page_content,
                    )
                if hasattr(doc, "vector") and doc.vector is not None:
                    attributes[
                        f"full_text_search.document.{i}.vector_size"
                    ] = len(doc.vector)
                if hasattr(doc, "provider"):
                    attributes[f"full_text_search.document.{i}.provider"] = (
                        doc.provider
                    )
                if hasattr(doc, "metadata"):
                    attributes[f"full_text_search.document.{i}.metadata"] = (
                        str(doc.metadata)
                    )
        return attributes
    ...
    def __call__(
        ...
        with self.tracer.start_as_current_span(
            ...
            try:
                ...
                logger.debug(
                    "Failed to set output attributes in FullTextSearchHandler",
                    exc_info=True,
                )
