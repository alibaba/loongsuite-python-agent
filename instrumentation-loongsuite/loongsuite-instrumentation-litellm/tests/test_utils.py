...
class TestParseProviderFromModel(unittest.TestCase):
    ...
    def test_model_with_slash_returns_provider_prefix(self):
        ...
        self.assertEqual(parse_provider_from_model("dashscope/qwen-turbo"), "dashscope")
        self.assertEqual(parse_provider_from_model("anthropic/claude-3"), "anthropic")
        self.assertEqual(parse_provider_from_model("google/gemini-pro"), "google")
        self.assertEqual(parse_provider_from_model("custom-provider/some-model"), "custom-provider")

    def test_model_with_multiple_slashes_returns_first_part(self):
        """Test that model with multiple '/' returns only the first part."""
        self.assertEqual(parse_provider_from_model("openai/gpt-4/turbo"), "openai")
        self.assertEqual(parse_provider_from_model("provider/model/version/extra"), "provider")
    ...
    def test_claude_model_inferred_as_anthropic(self):
        ...
        self.assertEqual(parse_provider_from_model("claude-3-opus"), "anthropic")
        self.assertEqual(parse_provider_from_model("CLAUDE-instant"), "anthropic")
    ...
    def test_unknown_model_returns_unknown(self):
        ...
        self.assertEqual(parse_provider_from_model("some-random-model"), "unknown")
