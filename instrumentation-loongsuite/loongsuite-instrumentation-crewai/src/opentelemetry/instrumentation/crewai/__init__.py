...
import time
import threading
from typing import Any, Collection, Dict, Optional, List
...
_CREWAI_LITELLM_RESPONSE_KEY = context_api.create_key("crewai_litellm_response")

def _safe_json_dumps(obj: Any, default: str = "", max_size: int = MAX_MESSAGE_SIZE) -> str:
    """
...
class _CrewKickoffWrapper:
    ...
    def __call__(self, wrapped, instance, args, kwargs):
        ...
        with self._tracer.start_as_current_span(
            ...
            span.set_attribute(CommonAttributes.GEN_AI_SPAN_KIND, GenAiSpanKind.CHAIN.value)
            span.set_attribute(CommonAttributes.GEN_AI_OPERATION_NAME, crew_name)
            span.set_attribute(CommonAttributes.INPUT_VALUE, _safe_json_dumps(inputs, "{}"))
            ...
            span.set_attribute(CommonAttributes.OUTPUT_VALUE, _safe_json_dumps(output_value, str(output_value)))
...
class _FlowKickoffAsyncWrapper:
    ...
    def __call__(self, wrapped, instance, args, kwargs):
        ...
        with self._tracer.start_as_current_span(
            ...
            span.set_attribute(CommonAttributes.GEN_AI_SPAN_KIND, GenAiSpanKind.CHAIN.value)
            span.set_attribute(CommonAttributes.GEN_AI_OPERATION_NAME, flow_name)
            span.set_attribute(CommonAttributes.INPUT_VALUE, _safe_json_dumps(inputs, "{}"))
            ...
            span.set_attribute(CommonAttributes.OUTPUT_VALUE, _safe_json_dumps(result, str(result)))
            span.set_status(Status(StatusCode.OK))
...
class _AgentExecuteTaskWrapper:
    ...
    def __call__(self, wrapped, instance, args, kwargs):
        ...
        with self._tracer.start_as_current_span(
            ...
            span.set_attribute(CommonAttributes.GEN_AI_SPAN_KIND, GenAiSpanKind.AGENT.value)
            ...
            if task:
                ...
                if tools:
                    input_data["tools"] = [getattr(t, "name", str(t)) for t in tools] if isinstance(tools, list) else str(tools)
                
                span.set_attribute(CommonAttributes.INPUT_VALUE, _safe_json_dumps(input_data, task_desc))
            else:
            ...
            span.set_attribute(CommonAttributes.OUTPUT_VALUE, str(result) if result else "")
            span.set_status(Status(StatusCode.OK))
...
class _TaskExecuteSyncWrapper:
    ...
    def __call__(self, wrapped, instance, args, kwargs):
        ...
        with self._tracer.start_as_current_span(
            ...
            span.set_attribute(CommonAttributes.GEN_AI_SPAN_KIND, GenAiSpanKind.TASK.value)
...
class _ToolUseWrapper:
    ...
    def __call__(self, wrapped, instance, args, kwargs):
        ...
        with self._tracer.start_as_current_span(
            ...
            # Use semantic convention constants (Rule 8)
            span.set_attribute(CommonAttributes.GEN_AI_SPAN_KIND, GenAiSpanKind.TOOL.value)
            
            # Set tool name (both old and new conventions for compatibility)
            span.set_attribute("tool.name", tool_name)  # Old convention
            span.set_attribute(ToolAttributes.GEN_AI_TOOL_NAME, tool_name)  # New convention
            
            # Set tool description
            if tool and hasattr(tool, "description"):
                description = tool.description
                span.set_attribute("tool.description", description)
                span.set_attribute(ToolAttributes.GEN_AI_TOOL_DESCRIPTION, description)
            ...
            if tool_calling and hasattr(tool_calling, "arguments"):
                ...
                span.set_attribute("tool.parameters", arguments_json)  # Old convention
                span.set_attribute(ToolAttributes.TOOL_PARAMETERS, arguments_json)  # Also use new name
            try:
                result = wrapped(*args, **kwargs)
            except Exception as e:
                span.record_exception(e)
            ...
            if instance:
                ...
                _max_parsing_attempts = getattr(instance, "_max_parsing_attempts", None)
